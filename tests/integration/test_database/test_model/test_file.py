...
class TestFile:
    ...
    def test_embedded(self) -> None:
        ...
        assert File.from_id(1).to_json(join=True) == {
            ...
            'epoch': Epoch.from_id(2).to_json(join=True),
    ...
    def test_relationship_epoch(self, testdata: TestData) -> None:
        """Test epoch foreign key relationship on file."""
        for i, record in enumerate(testdata['file']):
            assert File.from_id(i + 1).epoch.id == record['epoch_id']

    def test_relationship_type(self, testdata: TestData) -> None:
        """Test type foreign key relationship on file."""
        for i, record in enumerate(testdata['file']):
            assert File.from_id(i + 1).type.id == record['type_id']
