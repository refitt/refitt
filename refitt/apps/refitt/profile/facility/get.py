...
from refitt.database.profile import Facility as Facility, FacilityNotFound
...
USAGE = f"""\
usage: {PROGRAM} {{ID | NAME}} [-x VAR] [-1] [--profile NAME] [-d | -v] [--syslog]
{__doc__}\
"""
...
class FacilityGet(Application):
    ...
    exceptions = {
        ...
        FacilityNotFound: functools.partial(log_and_exit, logger=log.critical,
                                            status=exit_status.runtime_error),
    }
    ...
    def run(self) -> None:
        ...
        try:
            profile = Facility.from_id(int(self.source))
        except (ValueError, TypeError):
            profile = Facility.from_name(self.source)

        data = profile.to_dict()
        if self.attribute is not None:
            if self.attribute not in data:
                raise RuntimeError(f'"{self.attribute}" not an attribute of facility '
                                   f'"{self.source}"')
    ...
    def __enter__(self) -> FacilityGet:
