...
from refitt.server.app import application as app
from refitt.server.route import INFO
...
PROGRAM = 'refitt-server'
USAGE = f"""\
Usage:
  refitt-server [-h] {{start}} [-p PORT] [-w NUM] [-t SECONDS] [--dev]
                [--keyfile PATH] [--certfile PATH]
...
class ServerApp(Application):
    ...
    def run_gunicorn(self) -> None:
        ...
        cmd += cert_ops + ['refitt.server:app']
        subprocess.run(cmd, stdout=sys.stdout, stderr=sys.stderr)


def main(argv: List[str] | None = None) -> int:
    """Entry-point for server app."""
    return ServerApp.main(argv or sys.argv[1:])
