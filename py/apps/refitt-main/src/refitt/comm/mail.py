...
# public interface
__all__ = ['UserAuth', 'Mail', 'MailServer', 'MailTemplate',
           'TestMail', 'RecommendationMail', 'StatusMail',
...
<html>
    ...
    <body>
        ...
        Log in at <a href="http://refitt.org">refitt.org</a> for details.
        </p>
        <br>
    </body>
</html>
...
SERVICE_LISTING_TEMPLATE = """\
        <h2 style="color:{color}">● {name}: {state} ({pid} - {uptime})</h2>
"""


class StatusMail(MailTemplate):
    """Simple mail for report status of daemon services."""
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        for field in ('attach', 'text', 'html', 'subject'):
            value = kwargs.pop(field, None)
            if value is not None:
                raise AttributeError(f'Cannot specify \'{field}\' for StatusMail')
        ...
        for name, status in info.items():
            service_listing.append(
                SERVICE_LISTING_TEMPLATE.format(name=name, uptime=status['uptime'], pid=status['pid'],
                                                state=('alive' if status['alive'] else 'dead'),
                                                color=('green' if status['alive'] else 'red'))
            )

        super().__init__(*args, **kwargs)
        self.subject = f'REFITT Daemon ({self.date})'
        self.html = STATUS_TEMPLATE.format(service_listing='\n'.join(service_listing),
                                           date=self.date)
