...
"""Making the setup for interpolating the grid of models"""

models = pd.read_csv('./master_model_set.csv')
names = models['Model_name']
n = models[['ZAMS','Energy','Mass_Loss','Beta','M_Ni56']]
points = n.to_numpy()

"""Scaling the points dataframe for sensible output values"""

new_points = pd.DataFrame(points)
new_points[2] = -1*np.log10(new_points[2])
new_points = np.array(new_points)

"""Referencing to individual band files where there is fine grid of models for weighted interpolation"""

delta = np.array([0.01,0.01,0.01,0.01,0.001])
fine_epoch_g = pd.DataFrame(np.load('./fine_g_master.npy',allow_pickle =True))
fine_epoch_r = pd.DataFrame(np.load('./fine_r_master.npy',allow_pickle =True))

"""Defining Prior And Likelihood Functions For Samping"""

def prior_transform(theta):
    zamsp, Ekp, m_lossp, betap, ni_massp, texpp, a_vp = theta #zams, Ek, m_loss, beta,Ni_mass,texplosion
    zams_min = 12.0
    zams_max = 16.0
    zams_mu, zams_sig = 13, 2.0
    ...
    return (zams, Ek, m_loss, beta, ni_mass, texp, av)

#df = [g_cor,r_cor]
#bands = ['g','r']


def loglikelihood_dynesty(theta):#currently doing for g and r  bands = [g,r], df = [g_cor,r_cor]
    zams, Ek, m_loss, beta, ni_mass, texp, a_v = theta
    mag_list = []
    magerr_list = []
    mag_th = []
    for band in available_bands:
        new_epo, mag, mag_err = epoch_gen(df_cor[band],texplosion = theta[5], host = theta[6])
        mag_theory = interpol_LC_master_scaled(theta,band,new_epo,names,new_points, delta,fine_epoch_g,fine_epoch_r)
        mag_list.extend(mag)
        magerr_list.extend(mag_err)
        mag_th.extend(mag_theory)
        #print(np.shape(mag_th))
...
fig = corner.corner(samples_dynesty,show_titles=True,labels=['ZAMS','Energy','Mass_Loss','Beta','M_Ni56','t_exp','A_v'], quantiles=[0.16, 0.5, 0.84])
...
mjd_app, mag_app = obj.make_plots(results, df_cor,upper_limit,available_bands,upper_limit_bands, samples_dynesty, output,names,new_points,delta,fine_epoch_g,fine_epoch_r)
obj.make_json(results,samples_dynesty, output,mjd_app,mag_app,band_list= available_bands)
